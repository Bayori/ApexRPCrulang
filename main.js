/* Разработчик: Holfz https://github.com/Holfz*/
/* Перевод: Bayori https://github.com/Bayori*/
/* env */
...
logger.info('Попытка входа...', 'main:steam');
SteamClient.logOn({
...
SteamClient.on('error', function(e) {
    if (e.eresult == 5) {
        return logger.error('Неудачная попытка входа, перепроверьте введённый пароль.', 'main:steam');
    }

    return logger.error(`Неудачная попытка входа, код ошибки: ${e.eresult}`, 'main:steam');
});
...
SteamClient.on('steamGuard', async function(domain, callback) {
    ...
    if (!response || !response.code) {
        SteamClient.logOff();
        return logger.error('Вы не ввели код Steam Guard.', 'main:steam');
    }
...
SteamClient.on('loggedOn', async function(details) {
    logger.info(`Успешная попытка входа в аккаунт Steam: Добро пожаловать, ${details.vanity_url}.`, 'main:steam');
    SteamClient.setPersona(SteamUser.EPersonaState.Online);

    if (!['yes', 'y', 'true'].includes(process.env.LAUNCH_APEX_IF_NESSESARY)) {
        logger.debug(`Значение \`LAUNCH_APEX_IF_NESSESARY\` в вашем файле env не установлено.`, 'main:steam');
        return;
    }

    const r5apex = await find('name', 'r5apex', true);
    if (r5apex.length > 0) {
        logger.debug(`Apex Legends уже запущен.`, 'main:steam');
        return;
    }
    ...
    steamReg.values((err,res) => {
        if (err){
            logger.debug(`Steam не установлен.`, 'main:steam');
            return;
        }

        logger.debug(`Автоматический запуск Apex Legends, так как установлено значение \`LAUNCH_APEX_IF_NESSESARY\` в вашем файле env.`, 'main:steam');
        exec('start "" steam://run/1172470', (error, stdout, stderr) => {
            if (error) {
                return logger.error(`Неудачная попытка запуска Apex Legends: ${error.message}`);
            }

            logger.info('Apex Legends запущен!', 'main:steam')
        });
    });
});
...
SteamClient.on('playingState', function(blocked, playingApp) {
    if (playingApp == 1172470) {
        logger.info(`Вы запустили Apex Legends (AppID: ${playingApp}), запускаю DiscordRPC.`, 'main:steam');
        if (!discordReady) { RPC.login({ clientId: "893911040713191444" }); }
    } else {
        logger.info(`Вы прекратили играть в Apex Legends (AppID: ${playingApp}), останавливаю discord RPC.`, 'main:steam');
        if (discordReady) { RPC.destroy(); }
    }
});

SteamClient.on('disconnected', function(eresult, msg) {
    return logger.warn(`Отключение, код ошибки: ${Number(eresult)}`, 'main:steam');
});
...
SteamClient.on('user', function(sID, user) {
    ...
    if (!status && !steam_player_group_size) {
        ...
        if (
            ...
            if (playState == 2) {
                if (squadsleft) { activity.details += ` [Топ-${squadsleft.value}]`; }
            } else {
                activity.details += ` (${playState == 1 ? "Выбирает легенду" : "Просмотр сводки"})`;
            }
        }
        ...
        activity.details = Translation[status.value] ? Translation[status.value] : "НЕИЗВЕСТНО, СВЯЖИТЕСЬ С HOLFZ";
        if (!Translation[status.value]) {
            logger.warn(`НЕИЗВЕСТНО: ${status.value}`, 'main:user:rpc');
            logger.warn(`Отправьте эту информацию разработчику holfz (на английском): `);
            console.log(user.rich_presence);
        }
    }

    if (steam_player_group_size) {
        activity.state = (steam_player_group_size.value > 1) ? "В команде" : "Без команды";
        [activity.partySize, activity.partyMax] = [Number(steam_player_group_size.value), 3]; // NOTE: THIS IS HARDCODED
    } else {
        activity.state = `Команда не собрана`;
    }
...
/* main:RPC */
RPC.on('ready', () => {
    logger.info('Discord RPC запущен.', 'main:RPC');
    discordReady = true;
});